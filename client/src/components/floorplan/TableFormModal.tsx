  ...
  return (
    <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/30 p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-sm rounded-lg bg-white p-5 shadow-lg">
        <h2 className="mb-4 text-lg font-semibold text-gray-900">{initial ? `Edit table ${initial.number}` : "Add table"}</h2>

        {error && <div className="mb-3 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{error}</div>}

        <label className="mb-3 block text-sm">
          <span className="mb-1 block font-medium text-gray-700">Table number</span>
          <input
            type="number"
            required
            min={1}
        ...
        <label className="mb-3 block text-sm">
          <span className="mb-1 block font-medium text-gray-700">Seat capacity</span>
          <input
            type="number"
            required
            min={1}
        ...
        <label className="mb-5 block text-sm">
          <span className="mb-1 block font-medium text-gray-700">Shape</span>
          <select
            value={shape}
